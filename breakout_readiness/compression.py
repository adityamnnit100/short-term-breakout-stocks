...
def _safe_pct_change(current: float, reference: float) -> float:
...
def score_compression(df: pd.DataFrame) -> Tuple[float, Dict[str, float]]:
    ...
    atr_contraction_pct = _safe_pct_change(atr_recent, atr_prior)
    ...
    bbw_contraction_pct = _safe_pct_change(bbw_recent, bbw_prior)

    recent_std = float(close.tail(10).std() or 0.0)
    prior_std = float(close.tail(30).std() or 0.0)
    std_contraction_pct = _safe_pct_change(recent_std, prior_std)
    ...
    range_contraction_pct = _safe_pct_change(recent_range, prior_range)
