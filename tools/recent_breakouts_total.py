"""Scan recent breakout events (past 30 trading days) across Nifty Total Market.

Same output as recent_breakouts.py but uses the broader total market universe.
"""
import sys
import json
...
OUT_CSV = 'tools/recent_breakouts_total.csv'

def analyze():
    tickers = breakout.get_nifty_total_market()
    # Fetch 3 months of data to cover 30 trading days
    period = '3mo'
    interval = '1d'

    print('Downloading data for', len(tickers), 'tickers...')
    data = yf.download(tickers, period=period, interval=interval, progress=False, threads=False, timeout=240)
    ...
    results = []
    quality = breakout._scanner_quality_profile('Pre-Breakout', 'Total Market (Cap Focused)')
